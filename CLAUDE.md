...
- `GET /client/cups/all-time` — the record books: `best_weeks` (settled-week pots, raw entries so one user can hold several places, competition-ranked like the cup and cut at rank 5 so a tie is never split) and `profit_streak_record` (`{length, holders[]}` — the longest run of consecutive profitable settled weeks, every joint holder, `is_current` when it's their live run; `null` until someone has strung two together). Money as strings. A plain `def` route (threadpool), unlike the `async def` client routes above. `GET /client/me` also carries `best_week: {balance, week_start} | null`.
...
- **Sync by default.** Everything that touches the DB, `requests` or `firebase_admin` is a plain `def` so FastAPI runs it on the anyio threadpool. An `async def` handler runs *on the event loop*, where one blocking call freezes every request on the instance, `/health` included — that is what turned a slow upstream into a 12-hour outage on 2026-09-01. `src/tests/client/test_dispatch.py` enforces this for the client router. The only `async def` route is `ask_pundit`, which awaits an OpenAI stream; its DB prelude lives in the `pundit_context` `def` dependency. Admin views are the exception that proves the rule: SQLAdmin's `@expose` wraps them in an async `login_required` that calls a sync view inline on the loop, so they stay `async def` and hop with `run_in_threadpool` themselves.
- **Job runs are serialised, and the outbound ones are bounded.** `rapid_api/runner.py` owns the one `threading.Lock`; `run_jobs` (cron) and `run_job` (admin triggers) both claim it and return `False` if a run is in progress. `RUN_BUDGET_SECONDS` stops new jobs starting and `JOB_BUDGET_SECONDS` stops `fetch_fixtures`/`fetch_odds` starting new calls, so a *stalled upstream* can't produce a run that outlives Cloud Run's 300s and turns every later tick into a silent skip. The settlement and close jobs have no deadline — they're bounded by row caps, and the cup backstop loops not at all, so a large enough backlog could still run long. Anything deferred is picked up next tick: every job re-selects its work from row state.
...
- **First-write races are accepted** (friends-scale v1). `create_bet`'s balance-check-then-`entry.debit` *is* row-locked — `db.refresh(entry, with_for_update=True)` in `create_bet` itself, deliberately not inside `get_or_create_entry`, which commits when it creates an entry and would drop the lock before the caller used it. A `get_or_create` of a cup, entry or pundit-usage row can still lose a race on its unique constraint. The DB serialises the pair, so you never get two rows — the loser gets a transient 500 that rolls back cleanly and works on retry. Reachable only on a user's first bet of the week, or the week's first bet if the Monday 00:05 tick failed. Add an `IntegrityError` retry if it ever shows up.
- **The job lock is process-local.** `threading.Lock` covers one instance; Cloud Run allows two, and the second appears exactly when the first is wedged or during revision rollover. Two concurrent settlement passes could both read a bet as `UNDECIDED` and double-credit a win. Fix is a lease column on `JobControl` claimed in one atomic `UPDATE` (see `docs/audit-2026-09-02/` on the `audit` branch, action #8).
- **The Neon provider is pinned to `~> 0.13.0`, and that's deferred work.** 0.14 removed `neon_endpoint.pooler_enabled` and exposes the pooled host as a separate computed `host_pooling` attribute. `local.database_url` (`neon.tf`) reads `neon_endpoint.app.host`, which returns the *pooled* host only because the pooler is enabled — so upgrading without switching to `host_pooling` would silently repoint `DATABASE_URL` at the direct endpoint, changing the connection ceiling the app's 20-connection pool sits under. Same class as the `-pooler-pooler` bug already recorded in that file. Migrating means: bump the constraint, swap the attribute, and confirm the resulting host still ends `-pooler`.
- **No `statement_timeout`.** The engine sets `connect_timeout=5` but nothing bounds a slow query; post-sync-conversion that costs one thread rather than the instance. Alembic builds its own engine (`alembic/env.py`) and has no connect timeout either — its failure mode is a container that won't start, i.e. a visible failed deploy.
